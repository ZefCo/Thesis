...
class EE:
    ...
    def import_settings(self):
        ...
        self.show_pca: bool = True
        self.export_pca: bool = True

        self.show_lda: bool = True
        self.export_lda: bool = True
    ...
    def pca_output(self):
        ...
        if self.pca_output_on:
            ...
            pca_sta_plotly = px.scatter(pca_sta_plot, x = 'x', y = 'y', color = "Set", symbol = "Set", title = f"PCA of Statistics Data")

            pca_3ta_plotly = px.scatter_3d(pca_sta_plot, x = 'x', y = 'y', z = 'z', color = "Set", symbol = "Set", title = f"PCA of Statistics Data")
            ...
            if self.export_pca:
                pca_sta_plotly.write_html(pathlib.Path.cwd() / "PCA_2D_Scatter.html")
                pca_3ta_plotly.write_html(pathlib.Path.cwd() / "PCA_3D_Scatter.html")
    ...
    def random_comparison(self, agene, bgene):
        ...
        # Instead of generating a random gene: just take the 25% chance of it being the same as 1 and 75% as different being 0,
        # then multiply by self.weights
        rgene = numpy.array([0 if random.random() > 0.75 else 1 for _ in range(len(agene))])
        rg_num = numpy.sum(rgene * self.weight)
    ...
    def weights(self):
        '''
        Initializes the weights and another important matrix. More of an initialization method then a weights method
        '''
