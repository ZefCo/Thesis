...
for row in range(start_position, ut_rows):
    ...
    hg19_subdata = hg19_data[(hg19_data['name2'] == head_name) | (hg19_data['name2'] == tail_name)]

    # I like sets, they're fast
    hg19_names = set(hg19_subdata['name'])

    print(f"### {head_name}_{tail_name} ###")

    if len(full_seq) > window_size * 3:

        head_seq = full_seq[0:window_size]
        tail_seq = full_seq[len(full_seq) - window_size: len(full_seq)]

        head_gene_names: list or None = query_result(head_seq, hg19_names)
        tail_gene_names: list or None = query_result(tail_seq, hg19_names)

        # Draw this out, then update. I'm getting blank lists in the UT Database and I don't want that. I think something is being removed that shouldn't.

        # if not isinstance(head_gene_names, list):
        #     head_gene_names = ["Random Thing in this list to make sure it cant find anything"]
        # if not isinstance(tail_gene_names, list):
        #     tail_gene_names = ["That way I can just use the next line without having to worry about dataframe.isin(None) crashing the script"]
        ...
        if (isinstance(head_gene_names, list)) and (isinstance(tail_gene_names, list)):
            if (len(head_gene_names) > 0) and (len(tail_gene_names) > 0):
                ...
                print(f"Length of Head Gene List: {len(head_gene_names)}\nLength of Tail Gene List: {len(tail_gene_names)}\nWill be skipping this one due to at least one legnth being 0")
