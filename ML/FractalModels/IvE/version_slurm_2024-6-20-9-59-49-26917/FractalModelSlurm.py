...
norm_const = 255_000 # in case this number needs to be adjusted. It worked well for 64 and 256 sized lattices
batch_size = 250
epochs = 1000
warm_up_epochs = 20
base_filters = 5
...
def normalization(tensor_image, tensor_label):
    tensor_image = tf.cast(tensor_image / norm_const, tf.float32)
...
cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath = str(model_dir), save_best_only = True)
es_callback = tf.keras.callbacks.EarlyStopping(monitor = "val_loss", patience = 5, restore_best_weights = True, start_from_epoch = warm_up_epochs, mode = "min")
...
# Input layer, 256x256x1 -> 128x128x8
filters = base_filters * multiply_filters
a = tf.keras.layers.Conv2D(filters, (3, 3), padding = "same", activation="relu")(input_layer) #), activation = "gelu", kernel_regularizer = tf.keras.regularizers.l2(l = 0.001))(input_layer)
a = tf.keras.layers.MaxPooling2D(pool_size = (2, 2))(a)
a = tf.keras.layers.BatchNormalization()(a)

# First convolutional layer, 128x128x8 -> 64x64x16
filters = filters * multiply_filters
b = tf.keras.layers.Conv2D(filters, (3, 3), padding = "same", activation="relu")(a)
b = tf.keras.layers.MaxPooling2D(pool_size = (2, 2))(b)
b = tf.keras.layers.BatchNormalization()(b)

# Second convolution layer, 64x64x16 -> 32x32x32
filters = filters * multiply_filters
c = tf.keras.layers.Conv2D(filters, (3, 3), padding = "same", activation="relu")(b)
c = tf.keras.layers.MaxPooling2D(pool_size = (2, 2))(c)
c = tf.keras.layers.BatchNormalization()(c)

# Third convolutional layer, 32x32x32 -> 16x16x64
filters = filters * multiply_filters
d = tf.keras.layers.Conv2D(filters, (3, 3), padding = "same", activation = "relu")(c)
d = tf.keras.layers.MaxPooling2D(pool_size = (2, 2))(d)
d = tf.keras.layers.BatchNormalization()(d)

# Fourth convolutional layer, 16x16x64 -> 8x8x128
filters = filters * multiply_filters
e = tf.keras.layers.Conv2D(filters, (3, 3), padding = "same", activation = "relu")(d)
e = tf.keras.layers.MaxPooling2D(pool_size = (2, 2))(e)
e = tf.keras.layers.BatchNormalization()(e)

# Fifth convolutional layer, 8x8x128 -> 4x4x256
f = tf.keras.layers.Conv2D(256, (3, 3), padding = "same", activation = "relu")(e)
f = tf.keras.layers.MaxPooling2D(pool_size = (2, 2))(f)
f = tf.keras.layers.BatchNormalization()(f)

flatten = tf.keras.layers.Flatten()(f)
dense_1 = tf.keras.layers.Dense(100, activation = "relu", kernel_regularizer = tf.keras.regularizers.l2(l = 0.01))(flatten)
...
try:
    ...
    print(f"Output Directory of failed model:\n{version_dir}")
    exit()
