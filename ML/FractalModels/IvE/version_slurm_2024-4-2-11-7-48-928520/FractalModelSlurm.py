...
# Input layer, 64x64x1 -> 32x32x6
a = tf.keras.layers.Conv2D(6, (3, 3), padding = "same", activation="relu")(input_layer) #), activation = "gelu", kernel_regularizer = tf.keras.regularizers.l2(l = 0.001))(input_layer)
a = tf.keras.layers.MaxPooling2D(pool_size = (2, 2))(a)
a = tf.keras.layers.BatchNormalization()(a)

# First convolutional layer, 32x32x6 -> 16x16x36
b = tf.keras.layers.Conv2D(36, (3, 3), padding = "same", activation="relu")(a)
b = tf.keras.layers.MaxPooling2D(pool_size = (2, 2))(b)
b = tf.keras.layers.BatchNormalization()(b)

# Second convolution layer, 16x16x36 -> 8x8x216
c = tf.keras.layers.Conv2D(216, (3, 3), padding = "same", activation="relu")(b)
c = tf.keras.layers.MaxPooling2D(pool_size = (2, 2))(c)
c = tf.keras.layers.BatchNormalization()(c)

# Third convolutional layer, 8x8x216 -> 4x4x1269
d = tf.keras.layers.Conv2D(1296, (3, 3), padding = "same", activation = "relu")(c)
d = tf.keras.layers.MaxPooling2D(pool_size = (2, 2))(d)
d = tf.keras.layers.BatchNormalization()(d)

# Fourth convolutional layer, 4x4x625 -> 2x2x3125
# e = tf.keras.layers.Conv2D(5, (3, 3), padding = "same", activation = "relu")(d)
# e = tf.keras.layers.MaxPooling2D(pool_size = (2, 2))(e)
# e = tf.keras.layers.BatchNormalization()(e)

# de = tf.keras.layers.add([d, e])
# de = tf.keras.layers.MaxPooling2D(pool_size = (2, 2))(de)

# f = tf.keras.layers.Conv2D(7, (3, 3), padding = "same", activation = "gelu")(de)
# f = tf.keras.layers.BatchNormalization()(f)

flatten = tf.keras.layers.Flatten()(d)
dense = tf.keras.layers.Dense(50, activation = "relu", kernel_regularizer = tf.keras.regularizers.l2(l = 0.01))(flatten)
